...
namespace PFCharacters.Macros
{
	class SceneController : MonoBehaviour
	{
		...
		private async void Prepare()
		{
			...
			var buttonPrefab = await Addressables.LoadAssetAsync<GameObject>("Assets/PFCharacters/Prefabs/UI/Toggle.prefab").Task;
			foreach (CreatureSetting creature in CreatureList.creatures)
			{
				...
				var characterPrefab = await Addressables.LoadAssetAsync<GameObject>(creature.prefabAddress).Task;
				var instance = Instantiate(characterPrefab, PreviewCube, false);
				instance.transform.Rotate(0, 180, 0); // Y軸で180度回転
		...
		private async Task InstantiatePrefab()
		{
			...
			var characterPrefab = await Addressables.LoadAssetAsync<GameObject>(creature.prefabAddress).Task;
			CurrentModel = Instantiate(characterPrefab, MainCube, false);
			CurrentModel.transform.Rotate(0, 180, 0); // Y軸で180度回転
		}
